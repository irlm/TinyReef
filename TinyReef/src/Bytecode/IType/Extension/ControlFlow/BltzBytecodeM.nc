 ...
 * 
 */
module BltzBytecodeM
{
	provides interface TinyReefBytecode;
	
	uses
	{
		command void interpreterProgram(tinyReefContext_t* state, uint16_t programCounter); 
	}
}
...
implementation
{
	...
	command error_t TinyReefBytecode.execute(tinyReefContext_t *const context, uint32_t instruction)
	{
		...
		*/  
		...
		dbg("TinyReefBytecode", "TinyReefBytecode.execute - BltzBytecodeM\n");
