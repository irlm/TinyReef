...
implementation
{
    command error_t TinyReefBytecode.execute(tinyReefContext_t *const context, uint32_t instruction)
    {
		...
		*/   
		...
		dbg("TinyReefBytecode", "TinyReefBytecode.execute - LwrBytecodeM\n");
    	...
    	destination &= ~(MASK( (effectiveByteAddress)) << ((WORD_OFFSET - effectiveByteAddress) << WORD_OFFSET));
    	
    	for (var = effectiveByteAddress; var >= 0; var--) {
