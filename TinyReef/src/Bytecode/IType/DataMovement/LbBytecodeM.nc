...
implementation
{
    command error_t TinyReefBytecode.execute(tinyReefContext_t *const context, uint32_t instruction)
    {
		...
		*/   
		...
		dbg("TinyReefBytecode", "TinyReefBytecode.execute - LbBytecodeM\n");
    	...
    	context->registerFile[registerRT] = (int8_t) call Memory.getByte(context, context->registerFile[registerRS] + offset);
