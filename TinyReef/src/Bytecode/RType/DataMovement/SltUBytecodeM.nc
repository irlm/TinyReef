...
implementation
{
	command error_t TinyReefBytecode.execute(tinyReefContext_t *const context, uint32_t instruction)
    {
		...
		*/
		...
		dbg("TinyReefBytecode", "TinyReefBytecode.execute - SltUBytecodeM\n");
    	...
    	if ((uint32_t)context->registerFile[registerRS] < (uint32_t)context->registerFile[registerRT] )
    	{
