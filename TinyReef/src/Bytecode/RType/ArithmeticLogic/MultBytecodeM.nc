...
implementation
{
    command error_t TinyReefBytecode.execute(tinyReefContext_t *const context, uint32_t instruction)
    {
		...
		*/
		...
		dbg("TinyReefBytecode", "TinyReefBytecode.execute - MultBytecodeM\n");
		...
		result = (int32_t)context->registerFile[registerRS] * (int32_t)context->registerFile[registerRT];
